...
func lerArquivo(nomeArquivo string) ([]*Processo, error) {
	...
	for scanner.Scan() {
		...
		processo := &Processo{
			...
			tempoTermino:       -1, // -1 indica que ainda não terminou
		}
...
// novoSimulador cria um novo simulador com os processos fornecidos
func novoSimulador(processos []*Processo) *Simulador {
	return &Simulador{
		processos:      processos,
...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Println("Uso: go run main.go <arquivo_entrada>")
		fmt.Println("Exemplo: go run main.go processos.txt")
		os.Exit(1)
	}
	...
	simulador := novoSimulador(processos)
	simulador.executar()
