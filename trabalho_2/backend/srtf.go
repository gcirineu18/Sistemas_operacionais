...
func (alg *SRTF) adicionarProcessosNovos() {
...
func (alg *SRTF) executar() {
	...
	for {
		...
		for i := 0; i < tempoExecucao; i++ {
			...
			if len(alg.s.filaDeExecucao) > 0 && alg.s.filaDeExecucao[0].tempoRestante < processoAtual.tempoRestante {
